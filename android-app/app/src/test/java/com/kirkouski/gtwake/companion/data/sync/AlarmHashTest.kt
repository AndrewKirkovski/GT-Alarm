...
class AlarmHashTest {
    ...
    @Test
    fun `audioUri flows through canonical form but label does NOT`() {
        val alarm = Alarm(
            id = 42L,
            label = "Утро ☕",
            hour = 6,
        ...
        // The label is phone-only — never on the wire, never hashed — so the
        ...
        assertEquals(
            AlarmHash.compute(listOf(alarm)),
            AlarmHash.compute(listOf(alarm.copy(label = "a completely different label"))),
        )
    }
    ...
    fun `vibrationPattern alone changes the hash`() {
        val a = AlarmHash.compute(listOf(base()))
        val b = AlarmHash.compute(listOf(base().copy(vibrationPattern = com.kirkouski.gtwake.companion.domain.VibrationPattern.HEARTBEAT)))
